...
(function (global) {
  ...
  // 訂單查詢：對齊 v1 後台 ?action=queryOrder&phone=xxx
  // 回傳 { status:'success', orders:[{ timestamp, status, summary, details, delivery, total }] }
  async function queryOrder(phone) {
    const clean = String(phone || '').trim();
    if (!clean) return { status: 'error', message: '請輸入電話', orders: [] };

    if (IS_OFFLINE) {
      console.info('[ALU.api] queryOrder 離線模式，回示範訂單');
      // 離線示範：回一筆假單讓使用者看得到卡片長相
      return {
        status: 'success',
        orders: [{
          timestamp: new Date().toISOString(),
          status: 'unquoted',
          summary: '2020型 ×2、4040重型 ×1（示範訂單）',
          details: '【鋁材】2020型 [HR-2020L] (長度300mm) (x2)\n【鋁材】4040重型 [HR-4040L] (長度1200mm) (x1)',
          delivery: '小貨車配送',
          total: 560,
        }],
      };
    ...
    try {
      const url = `${API_URL}?action=queryOrder&phone=${encodeURIComponent(clean)}&t=${Date.now()}`;
      console.info('[ALU.api] queryOrder fetch:', url);
      const res = await fetch(url);
      const json = await res.json();
      console.info('[ALU.api] queryOrder 回應:', json && json.status, '筆數:', json && json.orders && json.orders.length);
      if (json && json.status === 'success') {
        return { status: 'success', orders: json.orders || [] };
      }
      return { status: 'error', message: (json && json.message) || '查詢失敗', orders: [] };
    } catch (e) {
      console.warn('[ALU.api] queryOrder failed:', e);
      return { status: 'error', message: '連線錯誤，請稍後再試', orders: [] };
    }
  }

  global.ALU = global.ALU || {};
  global.ALU.api = { getInventory, submitOrder, queryOrder, IS_OFFLINE, FALLBACK_PRODUCTS };
